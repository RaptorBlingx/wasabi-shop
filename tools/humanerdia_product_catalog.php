...
return [
    'ovos-skill' => [
        ...
        'description' => '<p><strong>HumanEnerDIA</strong> is an Open Voice OS skill for industrial energy management. It connects to the HumanEnerDIA/EnMS backend so operators can ask natural-language questions about factory energy performance, machine status, anomalies, forecasts, KPIs, and ISO 50001 action-plan context.</p>'
            . '<p><strong>Requirements:</strong> an OVOS-compatible runtime and a reachable HumanEnerDIA/EnMS API endpoint. The optional Qwen GGUF model is distributed separately and is not included in this download.</p>'
            . '<p><strong>Installation summary:</strong> extract the ZIP, set <code>ENMS_API_URL</code>, install the skill with <code>python3 -m pip install -e .</code>, start the OVOS runtime and REST bridge, then run the smoke query: <code>what is the power of compressor one</code>. For a clean-machine OVOS runtime experiment, use the companion OVOS Docker repository and point it at a reachable HumanEnerDIA backend.</p>'
