        ...
        if (isset($this->functions) || isset($this->constructor)) {
            ...
            if (empty($this->toAddress) && !empty($this->bytecode)) {
                ...
                foreach ($functions as $function) {
                    ...
                    try {
                //        file_put_contents(_PS_ROOT_DIR_ . '/RoyaltySystemActionLog.txt', PHP_EOL . '[' . date('Y-m-d H:i:s') . '] '. 'DataInside:'. json_encode($params). PHP_EOL . PHP_EOL, FILE_APPEND);
        
                        $data = $this->ethabi->encodeParameters($function, $params);
                        $functionName = Utils::jsonMethodToString($function);
              //          file_put_contents(_PS_ROOT_DIR_ . '/RoyaltySystemActionLog.txt', PHP_EOL . '[' . date('Y-m-d H:i:s') . '] '. 'DataInside2:'. $data. PHP_EOL . PHP_EOL, FILE_APPEND);
