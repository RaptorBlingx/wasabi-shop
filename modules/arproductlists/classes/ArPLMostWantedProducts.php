...
class ArPLMostWantedProducts extends ArPLListAbstract
{
    public $limit;
    public $days;
    public $more_link;
    ...
    public function getProducts($idLang)
    {
        ...
        if (Context::getContext()->controller instanceof CategoryController || Context::getContext()->controller instanceof CategoryControllerCore) {
            $currentCategory = Context::getContext()->controller->getCategory();
        }
        
        if ($this->category_filter && $currentCategory) {
            $join[] = 'LEFT JOIN `'._DB_PREFIX_.'category_product` cp ON cp.id_product = p.id_product';
            $conditions[] = 'cp.id_category = ' . (int)$currentCategory->id;
        }
        
        $join[] = ', (SELECT id_product, count(id_product) as carts
        ...
        $products = $this->getProducts2($id_lang, $id_shop, $this->limit, $conditions, $join, 'pv.carts', 'DESC', false, true, false);
