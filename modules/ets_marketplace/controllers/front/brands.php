...
class Ets_MarketPlaceBrandsModuleFrontController extends ModuleFrontController
{
    ...
    public function _initContent()
    {
        ...
        else
        {
            ...
            $paggination->limit =  (int)Tools::getValue('paginator_brand_select_limit',10);
            ...
            if($manufacturers)
            {
                ...
                foreach($manufacturers as &$manufacturer)
                {
                    ...
                    {
                        if($this->module->is17)
                            $manufacturer['logo'] = Module::getInstanceByName('ets_marketplace')->displayText('','img','','','','',$this->context->link->getManufacturerImageLink($manufacturer['id_manufacturer'],$type_image).'?time='.time());
                        else
                            $manufacturer['logo'] = Module::getInstanceByName('ets_marketplace')->displayText('','img','ets_mp_logo_mnf','','','',$this->module->getBaseLink().'/img/m/'.$manufacturer['id_manufacturer'].'.jpg?time='.time());
                    }
                    ...
                    $manufacturer['name'] =Module::getInstanceByName('ets_marketplace')->displayText($manufacturer['name'],'a','','',$this->context->link->getManufacturerLink($manufacturer['id_manufacturer']));
                }
            }
    ...
    public function renderListAddress($id_manufacturer)
    {
        ...
        $paggination->limit =  (int)Tools::getValue('paginator_address_select_limit',10);
    ...
    public function renderListProducts($id_manufacturer)
    {
        ...
        if($products)
        {
            ...
            foreach($products as &$product)
            {
                ...
                if($product['id_image'])
                {
                    $product['image'] = Module::getInstanceByName('ets_marketplace')->displayText(Module::getInstanceByName('ets_marketplace')->displayText('','img','width_80','','','',$this->context->link->getImageLink($product['link_rewrite'],$product['id_image'],$type_image)),'a','','',$this->context->link->getProductLink($product['id_product']));
                }
                else
                    $product['image']='';
                $product['name'] = Module::getInstanceByName('ets_marketplace')->displayText($product['name'],'a','','',$this->context->link->getProductLink($product['id_product']));
