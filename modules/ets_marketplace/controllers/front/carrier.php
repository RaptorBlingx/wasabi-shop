...
class Ets_MarketPlaceCarrierModuleFrontController extends ModuleFrontController
{
    ...
    public function _renderCarriersList()
    {
        ...
        $paggination->limit =  (int)Tools::getValue('paginator_carrier_select_limit',10);
        ...
        if($carriers)
        {
            foreach($carriers as &$carrier)
            {
                ...
                if(file_exists(_PS_SHIP_IMG_DIR_.$carrier['id_carrier'].'.jpg'))
                    $carrier['logo'] = Module::getInstanceByName('ets_marketplace')->displayText('','img','width_50','','','',$this->context->link->getMediaLink(_PS_IMG_.'s/'.$carrier['id_carrier'].'').'.jpg');
