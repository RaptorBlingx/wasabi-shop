...
class Ets_MarketPlaceManagerModuleFrontController extends ModuleFrontController
{
    ...
    public function initContent()
	{
        ...
        if(Tools::isSubmit('submitSaveManagerShop'))
        {
            ...
            if(!$this->module->_errors)
            {
                ...
                {
                    if($manager->active==-1)
                        $active = Module::getInstanceByName('ets_marketplace')->displayText($this->module->l('Pending','manager'),'span','ets_mp_status pending');
                    elseif($manager->active==0)
                        $active = Module::getInstanceByName('ets_marketplace')->displayText($this->module->l('Declined','manager'),'span','ets_mp_status declined');
                    elseif($manager->active==1)
                        $active = Module::getInstanceByName('ets_marketplace')->displayText($this->module->l('Approved','manager'),'span','ets_mp_status approved');
                    die(json_encode(
                        array(
                            'success' => $success,
    ...
    public function _initContent()
    {
        ...
        $paggination->limit =  (int)Tools::getValue('paginator_manager_select_limit',10);
        ...
        if($userManagers)
        {
            foreach($userManagers as &$userManager)
            {
                ...
                {
                    $userManager['active'] = Module::getInstanceByName('ets_marketplace')->displayText($this->module->l('Pending','manager'),'span','ets_mp_status pending');
                }
                elseif($userManager['active']==0)
                    $userManager['active'] = Module::getInstanceByName('ets_marketplace')->displayText($this->module->l('Declined','manager'),'span','ets_mp_status declined');
                elseif($userManager['active']==1)
                    $userManager['active'] = Module::getInstanceByName('ets_marketplace')->displayText($this->module->l('Accepted','manager'),'span','ets_mp_status approved');
            }
        }
