...
class Ets_MarketPlaceMySellerModuleFrontController extends ModuleFrontController
{
    ...
    public function postProcess()
    {
        ...
        if(!$this->context->customer->isLogged() ||  !Configuration::get('ETS_MP_ENABLED') || (!($registration = Ets_mp_registration::_getRegistration()) && Configuration::get('ETS_MP_REQUIRE_REGISTRATION') && !$this->seller))
    ...
    public function _initContent()
    {
        ...
        $seller_pages = array(
            ...
            'vacation' => array(
                'page' => 'vacation',
                'name' => $this->module->l('Vacation mode'),
            ),
            'manager' => array(
                'page' =>'manager',
        ...
        if(!Configuration::get('ETS_MP_ENABLE_CONTACT_SHOP'))
            unset($seller_pages['messages']);
        if(!Configuration::get('ETS_MP_VACATION_MODE_FOR_SELLER'))
            unset($seller_pages['vacation']);
        ...
        return $this->module->displayTpl('myseller.tpl');
    }
}