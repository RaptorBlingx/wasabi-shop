...
$(document).ready(function() {
	...
	$(document).on('click', 'a[href="#"], .list-group-item', function(e) {
		...
		var $btn = $(this),
			...
			response = function(r) {
				if ('errors' in r) {
					af.prependErrors($forms.find('.tab-title').first().next(), r.errors);
				} else if (r.saved) {
	...
	function prepareTemplate($template) {
		$template.on('click', '.removeFilter', function(){
			...
			var params = 'action=ShowAvailableFilters',
				response = function(r) {
					$('#dynamic-popup').find('.dynamic-content').html(r.content);
					$('#dynamic-popup').find('.modal-title').html(r.title);
				};
			ajaxRequest(params, response);
		...
		var params = 'action='+action+'Template&id_template='+idTemplate,
			response = function(r) {
				if ('errors' in r) {
					...
					$.growl.notice({ title: '', message: af_txt.saved});
					$parent.closest('.template-list').prepend(r.form_html).tooltip({selector: '.label-tooltip'});
				}
			};
	...
	$('.template-list').on('click', '.template-tab-option', function(e) {
		...
		let $parent = $(this).closest('.af_template'),
			...
			response = function (r) {
				if ('errors' in r) {
					af.prependErrors($parent, r.errors);
				} else {
	...
	$('.mergedGroup').on('change', function() {
		var $parent = $(this).closest('.merged-params'),
			...
			response = function(r) {
				if ('html' in r) {
					$parent.find('.merged-list').html(r.html).find('.basic-item').each(function(){
						updateSelectedOptionsTxt($(this));
					});
				}
			};
