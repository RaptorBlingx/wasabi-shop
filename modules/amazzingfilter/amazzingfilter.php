...
class AmazzingFilter extends Module
{
    ...
    public function definePublicVariables()
    {
        ...
        $this->page_link_rewrite_text = $this->is_modern ? 'page' : 'p';
    ...
    public function indexationTable($action)
    {
        ...
        switch ($action) {
            case 'install':
                ...
                $specific_types = ['w' => 'decimal(20,2)', 'd' => 'datetime', 'q' => 'tinyint(1)',
                'v' => 'tinyint(1)'];
    ...
    public function combinationPrices($action, $params = [], $force_action = false)
    {
        ...
        switch ($action) {
            ...
            case 'getVariableColumns':
                ...
                break;
            case 'extendCacheID':
                $ret = empty($this->i['p_identifier']) ? '' :
                    '_'.$this->context->country->id.'_'.$this->i['p_identifier'];
                break;
            case 'define':
                $ret = $params;
                $c_price_rows = empty($this->i['p_identifier']) ? [] : $this->db->executeS('
            ...
            case 'prepareQuery':
                ...
                switch ($params['order']['by']) {
                    ...
                    case 'manufacturer_name':
                        ...
                        break;
                    case 'date_upd':
                        $on = 'ps.id_product = i.id_product AND ps.id_shop = i.id_shop';
                        $query->select('ps.date_upd')->leftJoin('product_shop', 'ps', $on);
                        break;
                }
                foreach (['s', 'q', 'm'] as $c_name) {
                    if (isset($params['available_options'][$c_name])) {
                        $query->select($c_name); // possible repeating select(m) is removed in $query->build()
                    }
                }
    ...
    public function ajaxAction($action)
    {
        ...
        switch ($action) {
            ...
            case 'SaveAvailableCustomerFilters':
                ...
                $ret = ['success' => Configuration::updateValue('AF_SAVED_CUSTOMER_FILTERS', $filters)];
                break;
    ...
    public function getContent()
    {
        ...
        $to_unset = array_merge(['p', 'w'], array_keys($this->getSpecialFilters()));
    ...
    public function markBlockedIndexationFields(&$fields)
    {
        ...
        $check_values = ['p_c' => 'currency', 'p_g' => 'group', 'n' => 'lang', 't' => 'lang'];
    ...
    public function getGeneralSettingsFields()
    {
        ...
        if (Configuration::get('PS_ADVANCED_STOCK_MANAGEMENT')) {
            foreach (['oos_behaviour_', 'oos_behaviour', 'combinations_stock'] as $name) {
    ...
    public function oos($action, $params = [])
    {
        ...
        switch ($action) {
            ...
            case 'extendQuery':
                ...
                if ($params['exclude'] && $params['stock_mng']) {
    ...
    {
        return ['general', 'iconclass', 'themeclass', 'themeid', 'caching', 'indexation', 'seopage'];
    }
    ...
    public function getAvailableHooks()
    {
        ...
        foreach ($methods as $m) {
            if (Tools::substr($m, 0, 11) === 'hookDisplay' && !in_array($m, $methods_to_exclude)) {
                ...
                $available_hooks[$hook_name] = $hook_name;
            }
        }
        $registered_hooks = array_column($this->db->executeS('
            SELECT h.name FROM '._DB_PREFIX_.'hook_module hm
            LEFT JOIN '._DB_PREFIX_.'hook h ON (h.id_hook = hm.id_hook)
            WHERE h.name IN (\''.implode('\', \'', array_map('pSQL', $available_hooks)).'\')
            AND hm.id_module = '.(int)$this->id.' AND hm.id_shop IN ('.pSQL($this->getImplodedContextShopIds()).')
        '), 'name', 'name');
        $available_hooks = array_fill_keys($registered_hooks, 1) + array_fill_keys($available_hooks, 0);
    ...
    public function ajaxUpdateHook()
    {
        ...
        if ($ret['success'] = $this->registerHook($new_hook, $this->shop_ids)) {
    ...
    {
        $filters_data = array (
            'c' => ['type' => 1, 'nesting_lvl' => 0, 'foldered' => 1],
    ...
    public function saveTemplate(
        ...
        $encoded_filters_data = json_encode($filters_data);
        $encoded_additional_settings = json_encode($additional_settings);
        ...
        if (count($shop_ids) > 1 && $controller_ids) {
            ...
            $shop_ids = array_intersect($shop_ids, array_column($this->db->executeS('
                SELECT DISTINCT(id_shop) FROM ' . _DB_PREFIX_ . pSQL($shop_table) . '
        ...
        foreach ($shop_ids as $id_shop) {
            ...
            if (in_array($template_controller, $this->getControllersWithMultipleIDs())) {
                $controller_ids = $controller_ids ? $controller_ids : [0];
            ...
            foreach ($multilang_data as $id_lang => $data) {
                $encoded_lang_data = json_encode($data);
    ...
    **/
    ...
    public function hookDisplayHeader()
    {
        ...
        if ($this->defineFilterParams()) {
            ...
            $js_def = [
                'af_ajax_path' => $this->context->link->getModuleLink($this->name, 'ajax', ['ajax' => 1]),
    ...
    {
        if (!empty($this->sp)) {
            $this->sp->extendCurrentFilters($current_filters, $this->current_controller);
        }
    ...
    public function prepareInitialParams(&$filters)
    {
        $initial_params = array_fill_keys(
            ['available_options', 'numeric_slider_values', 'sliders', 'and'],
            []
        );
        foreach ($filters as $key => &$f) {
    ...
    public function prepareFiltersForDisplay($filters, &$params)
    {
        ...
        foreach ($filters as $key => &$f) {
            ...
            $remove_unused_options = in_array($f['first_char'], ['a', 'f', 'c']);
    ...
    {
        if ($f['first_char'] == 'c') {
            $caching_params = [$f['id_parent'], $f['nesting_lvl'], $this->id_shop, $this->id_lang];
            $caching_params = array_merge($caching_params, $this->formatIDs($this->c_groups));
        } else {
            $caching_params = [$f['id_group'], $this->id_shop, $this->id_lang];
        }
    ...
    public function validatePageNumber(&$params)
    {
        ...
        if ($params['page'] < 1 || ($params['page'] == 1 && Tools::isSubmit($this->page_link_rewrite_text)) ||
            ...
            $url = $this->updateQueryString($url, [$this->page_link_rewrite_text => $updated_page]);
    ...
    public function getNumReplacements()
    {
        if (!isset($this->num_replacements)) {
            $this->num_replacements = [];
            $standard_values = ['tho_sep' => '', 'dec_sep' => '.']; // tho before dec!
    ...
    */
    ...
    {
        if (!isset($this->x['generated_links'][$group][$link])) {
            $this->x['generated_links'][$group][$link] = 1;
        } elseif ($identifier) {
    ...
    public function getDataForPriceCalculation($id_shop)
    {
        if (!isset($this->i['p_data'][$id_shop])) {
            foreach (['group' => 'g', 'currency' => 'c'] as $name => $key) {
                ...
                $query->leftJoin('specific_price', 'sp', 'sp.'.pSQL($join_on).' AND sp.id_shop IN (s.id_shop, 0)');
    ...
    public function getDefaultSuffixes($resource, $id_shop)
    {
        ...
        if (isset($this->i['default'][$first_char])) {
            $shop_ids = $id_shop ? [$id_shop] : $this->all_shop_ids;
            foreach ($shop_ids as $id_shop) {
    ...
    public function updateIndexationData($product_ids, $id_shop, $params = [])
    {
        ...
        foreach ($product_ids as $id) {
            ...
            $row = ['id_product' => $id, 'id_shop' => $id_shop];
            ...
            foreach ($indexation_columns['variable'] as $c_name => $c_suffixes) {
                ...
                if ($this->i['p_comb'] && $c_name == 'p') {
                    $cp_rows += $this->combinationPrices('prepareRows', [
    ...
    public function renderLayout()
    {
        ...
        return $this->display(__FILE__, 'views/templates/front/basic-layout'.($this->is_modern ? '-17' : '').'.tpl');
    }
    ...
    public function getSelectedCombinations($product_ids, $selected_atts)
    {
        ...
        $raw_data = $this->db->executeS('
            ...
            WHERE pa.id_product IN ('.pSQL(implode(', ', $product_ids)).')
            AND pac.id_attribute IN ('.pSQL(implode(', ', $att_ids)).')
    ...
    */
    ...
    {
        $smarty_vars = ['applied_customer_filters' => []];
    ...
    public function getCustomerFilters($id_customer)
    {
        ...
        ');
        if ($customer_filters) {
            $customer_filters = json_decode($customer_filters, true);
        }
    ...
    public function ajaxSaveCustomerFilters()
    {
        ...
        ';
        $ret = ['success' => $this->db->execute($query)];
        exit(json_encode($ret));
    }
    ...
    public function getAdjustableCustomerFilters($decode = true)
    {
        ...
        if ($decode) {
            $adjustable_fitlers = $adjustable_fitlers ? json_decode($adjustable_fitlers, true) : [];
        }
