    ...
    <section id="content" class="ets-mp-page-content-seller">
        ...
        <form id="seller-register-form" action="" method="post" enctype="multipart/form-data"{if !isset($smarty.post.submitSeller)} style="display:none;{/if}">
            ...
            <section>
                ...
                {/if}
                {if in_array('latitude',$ETS_MP_REGISTRATION_FIELDS)}
                    <div class="form-group row">
                ...
                {/if}
                {if in_array('longitude',$ETS_MP_REGISTRATION_FIELDS)}
                    <div class="form-group row">
                ...
                {if in_array('banner_url',$ETS_MP_REGISTRATION_FIELDS)}
                    <div class="form-group row">
                        ...
                        <div class="col-md-9">
                            <input class="form-control banner_url" name="banner_url" type="text" id="banner_url" value="{if isset($smarty.post.banner_url)}{$smarty.post.banner_url|escape:'html':'UTF-8'}{/if}" />
                        </div>
                    </div>
                {/if}
                ...
                {/if}
                {if in_array('id_shop_category',$ETS_MP_REGISTRATION_FIELDS) && $shop_categories}
                    <div class="form-group row">
                        <label class="col-md-3 form-control-label{if in_array('id_shop_category',$ETS_MP_REGISTRATION_FIELDS_VALIDATE)} required{/if}"> {l s='Shop category' mod='ets_marketplace'} </label>
                        <div class="col-md-9">
                            <select class="form-control" id="id_shop_category" name="id_shop_category">
                                <option value="">--{l s='Choose shop category' mod='ets_marketplace'}--</option>
                                {foreach from =$shop_categories item='shop_category'}
                                    <option value="{$shop_category.id_ets_mp_shop_category|intval}" {if isset($smarty.post.id_shop_category) && $smarty.post.id_shop_category==$shop_category.id_ets_mp_shop_category}selected="selected"{/if}>{$shop_category.name|escape:'html':'UTF-8'}</option>
                                {/foreach}
                            </select>
                        </div>
                    </div>
