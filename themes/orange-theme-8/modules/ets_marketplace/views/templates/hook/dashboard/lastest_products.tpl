{*
 * Copyright ETS Software Technology Co., Ltd
 *
 * NOTICE OF LICENSE
 *
 * This file is not open source! Each license that you purchased is only available for 1 website only.
 * If you want to use this file on more websites (or projects), you need to purchase additional licenses.
 * You are not allowed to redistribute, resell, lease, license, sub-license or offer our resources to any third party.
 *
 * DISCLAIMER
 *
 * Do not edit or add to this file if you wish to upgrade PrestaShop to newer
 * versions in the future.
 *
 * @author ETS Software Technology Co., Ltd
 * @copyright  ETS Software Technology Co., Ltd
 * @license    Valid for 1 website (or project) for each purchase of license
*}
<table class="table">
    <thead>
        <tr>
            <th>{l s='ID' mod='ets_marketplace'}</th>
            <th>{l s='Image' mod='ets_marketplace'}</th>
            <th>{l s='Product name' mod='ets_marketplace'}</th>
            <th class="text-center">{l s='Price' mod='ets_marketplace'}</th>
            <th class="text-center">{l s='Quantity' mod='ets_marketplace'}</th>
            <th>{l s='Shop name' mod='ets_marketplace'}</th>
            <th class="text-center">{l s='Active' mod='ets_marketplace'}</th>
            <th class="text-center">{l s='Added date' mod='ets_marketplace'}</th>
            <th class="text-right">{l s='Action' mod='ets_marketplace'}</th>
        </tr>
    </thead>
    <tbody>
        {if $lastest_products}
            {foreach from=$lastest_products item='product'}
                <tr>
                    <td>{if $product.id_product}{$product.id_product|intval}{else}--{/if}</td>
                    <td>{if $product.image}{$product.image nofilter}{else}--{/if}</td>
                    <td>{if $product.name}{$product.name nofilter}{else}--{/if}</td>
                    <td class="text-center">{if $product.price}{$product.price|escape:'html':'UTF-8'}{/if}</td>
                    <td class="text-center">{$product.quantity|intval}</td>
                    {*<td class="seller_name">
                        {if $product.id_customer_seller}
                            <a href="{$module->getLinkCustomerAdmin($product.id_customer_seller)|escape:'html':'UTF-8'}&viewseller=1&id_seller={$product.id_seller|intval}">{$product.seller_name|escape:'html':'UTF-8'}</a>
                        {else}
                            {l s='Seller deleted' mod='ets_marketplace'}
                        {/if}
                    </td>*}
                    <td class="shop_name">
                        {if $product.id_seller}
                            <a href="{$module->getShopLink(['id_seller'=>$product.id_seller])|escape:'html':'UTF-8'}">{$product.shop_name|escape:'html':'UTF-8'}</a>
                        {else}
                            <span class="deleted_shop row_deleted">{l s='Shop deleted' mod='ets_marketplace'}</span>
                        {/if}
                    </td>
                    <td class="text-center">
                        {if $product.active}
                            <i class="fa fa-check" title="{l s='Active' mod='ets_marketplace'}"></i>
                        {else}
                            <i class="fa fa-remove" title="{l s='Disabled' mod='ets_marketplace'}"></i>
                        {/if}
                    </td>
                    <td class="text-center">{dateFormat date=$product.date_add full=1}</td>
                    <td class="text-right">
                        <a class="btn btn-default" href="{$product.link|escape:'html':'UTF-8'}">
                            <i class="icon-search-plus fa fa-search-plus"></i>
                            {l s='View' mod='ets_marketplace'}
                        </a>
                    </td>
                </tr>
            {/foreach}
        {else}
            <tr>
                <td colspan="100%" class="text-center no_data">{l s='No data' mod='ets_marketplace'}</td>
            </tr>
        {/if}
    </tbody>
</table>