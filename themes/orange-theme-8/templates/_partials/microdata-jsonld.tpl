...
{elseif $page.page_name === 'category' && isset($category) && isset($category.image) && isset($category.image.large) && $category.image.large.url}
...
<script type="application/ld+json">
{
    ...
    "potentialAction": {
      "@type": "SearchAction",
      "target": "{$urls.base_url}/ricerca?controller=search&s={literal}{search_term_string}{/literal}",
...
{if isset($product) && $page.page_name == 'product'}
    ...
    {if isset($product.cover)}"image" :"{$product.cover.bySize.medium_default.url}",{/if}
