        ...
        if (Validate::isLoadedObject($this->context->cart) && $this->context->cart->OrderExists() == false) {
            ...
            foreach ($order_detail_list as $key => $order_detail) {
                ...
                if (isset($order->id)) {
                    if (!$secure_key) {
                        if(version_compare(_PS_VERSION_, '1.7', '>='))
                            $message .= Module::getInstanceByName('ets_marketplace')->displayText('','br').$this->trans('Warning: the secure key is empty, check your payment account before validation', array(), 'Admin.Payment.Notification');
                        else
                            $message .= Module::getInstanceByName('ets_marketplace')->displayText('','br').Tools::displayError('Warning: the secure key is empty, check your payment account before validation');
                    }
                    if (isset($message) & !empty($message)) {
                        $msg = new Message();
                        $message = $message = strip_tags($message, '<br>');
                            ...
                            foreach ($customized_datas[$product['id_product']][$product['id_product_attribute']][$order->id_address_delivery] as $customization) {
                                ...
                                if (isset($customization['datas'][Product::CUSTOMIZE_TEXTFIELD])) {
                                    foreach ($customization['datas'][Product::CUSTOMIZE_TEXTFIELD] as $text) {
                                        $customization_text .= Module::getInstanceByName('ets_marketplace')->displayText($text['name'],'strong','').$text['value'].Module::getInstanceByName('ets_marketplace')->displayText('','br');
                                    }
                                }
                                if (isset($customization['datas'][Product::CUSTOMIZE_FILE])) {
                                    if(version_compare(_PS_VERSION_, '1.7', '>='))
                                        $customization_text .= $this->trans('%d image(s)', array(count($customization['datas'][Product::CUSTOMIZE_FILE])), 'Admin.Payment.Notification') . Module::getInstanceByName('ets_marketplace')->displayText('','br');
                                    else
                                        $customization_text .= sprintf(Tools::displayError('%d image(s)'), count($customization['datas'][Product::CUSTOMIZE_FILE])).Module::getInstanceByName('ets_marketplace')->displayText('','br');
                                }
                    ...
                    foreach ($cart_rules as $cart_rule) {
                        ...
                        if($cart_rule['obj']->reduction_amount && $cart_rule['obj']->reduction_product >0)
                        {
                            $ok = false;
                            foreach($order->product_list as $product)
                            {
                                if($product['id_product']==$cart_rule['obj']->reduction_product)
                                {
                                    $ok = true;
                                }
                            }
                    ...
                    $total_discount_tax_excl = (float)Db::getInstance()->getValue('SELECT SUM(value_tax_excl) FROM `'._DB_PREFIX_.'order_cart_rule` WHERE id_order='.(int)$order->id);
                    if($order->total_discounts_tax_excl!= $total_discount_tax_excl)
                    {
                        $total_discount_incl = (float)Db::getInstance()->getValue('SELECT SUM(`value`) FROM `'._DB_PREFIX_.'order_cart_rule` WHERE id_order='.(int)$order->id);
                        $order->total_paid_tax_excl +=($order->total_discounts_tax_excl- $total_discount_tax_excl);
                        $order->total_paid_tax_incl += ($order->total_discounts_tax_incl - $total_discount_incl);
                        $order->total_paid_tax_incl = Tools::ps_round($order->total_paid_tax_incl,6);
                        $order->total_paid_tax_excl = Tools::ps_round($order->total_paid_tax_excl,6);
                        $order->total_paid = Tools::ps_round($order->total_paid_tax_incl,6);
                        $order->total_discounts_tax_excl = Tools::ps_round($total_discount_tax_excl,6);
                        $order->total_discounts_tax_incl = Tools::ps_round($total_discount_incl,6);
                    ...
                    if ($id_order_state != Configuration::get('PS_OS_ERROR') && $id_order_state != Configuration::get('PS_OS_CANCELED') && $this->context->customer->id) {
                        ...
                        $data = array(
                            ...
                            '{delivery_block_html}' => $this->_getFormatedAddress($delivery, Module::getInstanceByName('ets_marketplace')->displayText('','br'), array(
                                'firstname' => Module::getInstanceByName('ets_marketplace')->displayText('%s','span','bold'),
                                'lastname' => Module::getInstanceByName('ets_marketplace')->displayText('%s','span','bold'),
                            )),
                            '{invoice_block_html}' => $this->_getFormatedAddress($invoice, Module::getInstanceByName('ets_marketplace')->displayText('','br'), array(
                                'firstname' => Module::getInstanceByName('ets_marketplace')->displayText('%s','span','bold'),
                                'lastname' => Module::getInstanceByName('ets_marketplace')->displayText('%s','span','bold'),
                            )),
